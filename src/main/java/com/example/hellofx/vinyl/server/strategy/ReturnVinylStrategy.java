...
public class ReturnVinylStrategy extends AbstractVinylStrategy {
    @Override
    ...
    public Map<String, Object> handle(RequestContext context, Map<String, Object> request) {
        ...
        context.getLibrary().returnVinyl(vinyl, user);
        if (context.getLibrary().findById(vinyl.getId()).isEmpty()) {
            return ServerMessages.success(request, "Vinyl returned and removed.");
        }
        return ServerMessages.vinylResponse(request, "Vinyl returned.", vinyl);
    }
}
